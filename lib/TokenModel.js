...
function movesTokenAwayFrom(
  ...
  return (
    tokensAreEquivalent(
      moveInteraction.token,
      moveAwayFromInteraction.firstToken
    ) &&
    tokenRelation.movedTokenIsFurtherFrom(moveAwayFromInteraction.secondToken)
  );
}
...
function putsTokenBetween(
  tokenRelation,
  moveInteraction,
  putBetweenInteraction
) {
  return (
    tokensAreEquivalent(
      moveInteraction.token,
      putBetweenInteraction.firstToken
    ) &&
    tokenRelation.movedTokenIsBetween(
      putBetweenInteraction.secondToken,
      putBetweenInteraction.thirdToken
    )
  );
}
...
export class TokenInteraction {
  ...
  submitSingleTokenInteraction(interaction, tokenRelation) {
    this.satisfied_ =
      ...
      (interaction.action === Action.move ||
        interaction.action === Action.pickUp)
