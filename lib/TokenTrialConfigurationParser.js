...
export function parseTokenInteractionRule(text) {
  const inOrderRules = text.split("\n").map((line) => {
    const inAnyOrderRules = line.split(",").map((interaction) => {
    ...
    return inAnyOrderRules.length > 1
      ? new InAnyOrder(inAnyOrderRules)
      : destructureSingleItem(inAnyOrderRules);
  });
  return inOrderRules.length > 1
    ? new InOrder(inOrderRules)
    : destructureSingleItem(inOrderRules);
}
