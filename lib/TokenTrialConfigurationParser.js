...
function dualTokenInteractionUsing(TokenInteractionType, match) {
...
function singleTokenInteractionUsing(TokenInteractionType, match) {
  return new TokenInteractionType({
...
function singleTokenInteraction(match) {
  ...
  return split.length === 2
    ? singleTokenInteractionUsing(TokenInteraction, match)
    : singleTokenInteractionUsing(SizedTokenInteraction, match);
}

function useToTouchTokenInteraction(match) {
  const split = match.groups.firstToken.split(" ");
  return split.length === 2
    ? dualTokenInteractionUsing(TokenInteraction, match)
    : dualTokenInteractionUsing(SizedTokenInteraction, match);
}

export function parseTokenInteractionRule(text) {
  const inOrderRules = destructureSingleItem(
    text.split("\n").map((line) => {
      const inAnyOrderRules = destructureSingleItem(
      ...
      return Array.isArray(inAnyOrderRules)
        ? new InAnyOrder(inAnyOrderRules)
        : inAnyOrderRules;
    })
  );
  return Array.isArray(inOrderRules) ? new InOrder(inOrderRules) : inOrderRules;
}
